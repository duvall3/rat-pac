...
bool kRaster = false;
//bool kRaster = true;
if (kRaster) {			// raster graphics
  settings.outformat="png";
  settings.render=2**3;
  //settings.render=2**4;
} else {			// vector graphics
...
transform TL = rotate(90);

label(TL*"$\overline{\nu_e}$", xl, SW, heavygreen);
label(TL*"$p$", yl, SE, black);

label(TL*"$e^+$", xu, NW, red);
label(TL*"$n$", yu, NE, heavyblue);

label(TL*"$W^+$", zl--zr, 1.5up, heavygray);

// indicate direction of time
drawMomArrow(tl--tr, 0.9*Relative(left));
label(TL*Label("$t$", 1.8LeftSide), tl--tr);
