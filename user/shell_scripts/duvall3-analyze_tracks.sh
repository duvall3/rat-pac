#!/bin/bash
# duvall3_analyze_tracks.sh -- wrapper script for ROOT analysis macros
...
# root-command defs
RTCMD_1=$(echo -e "root -q -l -b 'ibdTracksToScint.cxx(\"$DATARUN.root\")'")
...
# MAIN
if [[ -e "$DATARUN".root && ! -e "$DATARUN"_T.root ]]; then eval $RTCMD_1; fi
eval $RTCMD_2
eval $RTCMD_3
