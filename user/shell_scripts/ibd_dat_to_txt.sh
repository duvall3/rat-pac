...
echo "event positron_initial_KE positron_begin_x positron_begin_y positron_begin_z positron_step1_x positron_step1_y positron_step1_z positron_end_x positron_end_y positron_end_z positron_end_t positron_gammas positron_gamma_KE_total neutron_initial_KE neutron_begin_x neutron_begin_y neutron_begin_z neutron_step1_x neutron_step1_y neutron_step1_z neutron_end_x neutron_end_y neutron_end_z neutron_total_scatters neutron_end_t neutron_alphas neutron_alpha_KE neutron_gammas neutron_gamma_KE_total"
# NeutronCaptures.cxx currently uses the ROOT GlobalTime for 't', so 'begin_t' is zero by definition

paste \
	<(awk '$1 ~ /Event/ {print $2}' $1) \
	<(awk '$0 ~ /e\+ KE_0/ {print $3}' $1) \
