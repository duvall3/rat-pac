...
# help check
if [[ $1 =~ -h ]]; then
  $ECHO "USAGE: geogen_general-segmented.sh [-h|reset]" > $ERR
  $ECHO "Segmented-detector generation: Prompt for parameters, then create .geo file." > $ERR
  $ECHO "  -h: Show this help message and exit." > $ERR
  $ECHO "  No arguments: Run generator." > $ERR
  $ECHO "  Argument [reset]: Clear existing geometry.\n    -- WARNING: THIS WILL DELETE THE CURRENT DETECTOR." > $ERR
  $ECHO > $ERR
  exit 10
fi
...
echo -e "Detector geometry cleared.\nIf this was unintentional and you do not have a backup,\n  you should be able to recreate your geometry by running this script\n  and entering the parameters in $LOGFILE.\n  (If so, remember to divide the dimensions by 2)." && exit 0
  else
...
# don't overwrite
if [ -e $ARRFILE ]; then
  echo "ERROR: $ARRFILE already exists; please run 'geogen_general-segmented.sh reset' or remove the file manually if you are certain you want to define a new experiment geometry." && exit 11
fi
if [ -e $OUTFILE ]; then
  echo "ERROR: $OUTFILE already exists; please run 'geogen_general-segmented.sh reset' or remove the file manually if you are certain you want to define a new experiment geometry." && exit 12
fi
...
# print config
printf "### Geometry-creation log for %s ###\n# Generated by %s on %s using geogen_general-segmented.sh\n#   at %s\n" $PROJ $USER "$('hostname')" "$('date' -R)"  > $LOGFILE
printf "\nRows: %i\nColumns: %i\nLayers: %i\n" $ROWS $COLS $LYRS | tee -a $LOGFILE
printf "\nCell Length: \t%f mm\nCell Width: \t%f mm\nCell Height: \t%f mm\nCell Spacing: \t%f mm\n" $FL $FW $FH $FS | tee -a $LOGFILE
printf "\nArray Length: \t%f mm\nArray Width: \t%f mm\nArray Height: \t%f mm\n" $ca_Flength $ca_Fwidth $ca_Fheight | tee -a $LOGFILE
printf "\nTarget-cell material: \t%s\nMatrix material: \t%s\n\n" $TARGET_CELL_MATERIAL $ARRAY_MATERIAL | tee -a $LOGFILE
...
printf "\nGeometry generation complete.\nCreation log written to: $LOGFILE\n" | tee -a $LOGFILE
