...
echo "\
    ...
    ax_mult_by_agent = axes('color', .8*[1 1 1], 'xgrid', 'on', 'ygrid', 'on');
    T_mult_by_agent = title('Gamma Multiplicities for $DATARUN', 'interpreter', 'none');
...
for agent_file in ./gam/*.gam; do
  ...
  echo "\
    ...
    p_mult_$agent = stairs( mults-0.5, h_g_mult_$agent, 'color', cmap(k_agent,:) , 'linewidth', 2.0 ); % -0.5 to center hist. bars on integers
...
echo "\
  ...
  L_mult = legend( agent_mult_handles, agent_names, 'Location', 'NorthEastOutside', 'interpreter', 'none');
...
for agent_file in ./gam/*.gam; do
  ...
  echo "\
    ...
    p_ke_$agent = stairs( E, h_g_ke_$agent + .01, 'color', cmap(k_agent,:), 'linewidth', 2.0 ); % +.01 makes every count finite so ML will draw graph properly
...
echo "\
  ...
  set(ax_ke_by_agent, 'xgrid', 'on', 'ygrid', 'on')
  set(ax_ke_by_agent, 'color', .8*[1 1 1])
  YL = get(ax_ke_by_agent, 'ylim');
  ylim([10^0 YL(2)])
  
  L_ke = legend( agent_ke_handles, agent_names, 'Location', 'NorthEastOutside', 'interpreter', 'none' );
