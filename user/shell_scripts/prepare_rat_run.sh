#!/bin/bash
# prepare_rat_run.sh -- prepare and analyze data from a RAT-PAC (IBD) run
# ~ Mark J. Duvall ~ mjduvall@hawaii.edu ~ 9/2021 ~ #

## usage
if [ $# -lt 2 ]; then
  echo -e "\nUSAGE: prepare_rat_run.sh <DATARUN_NAME> <NUM_EVENTS> [GRAPHICS_TF]\n" && exit 10
fi
...
ROOTCOMMAND=$(echo -e "'$RATROOT/user/root_macros/duvallAnalyzePrep.cxx(\"$BASENAME\", $GRAPHICS_TF, \"$QUANTIZED_POSITIONS\", \"$POSITION_RESOLUTIONS\")'")
