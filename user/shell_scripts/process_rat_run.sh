#!/bin/bash
# process_rat_run -- prepare and analyze data from a RAT-PAC (IBD) run
# ~ Mark J. Duvall ~ mjduvall@hawaii.edu ~ August 2015 ~ #
# ~ Updated 8/21 ~ #

## usage
if [ $# -lt 2 ]; then
  echo -e "\nUSAGE: process_rat_run <DATARUN_NAME> <NUM_EVENTS> [GRAPHICS_TF] [FULL_ANALYSIS_TF]\n" && exit 10
fi
...
if [[ -z $4 ]]; then FULL_ANALYSIS_TF=true; else FULL_ANALYSIS_TF=$4; fi
...
ROOTCOMMAND=$(printf "'$RATROOT/user/root_macros/duvallAnalyze.cxx(\"$BASENAME\", $GRAPHICS_TF, $FULL_ANALYSIS_TF)'")
