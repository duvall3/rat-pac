...
<mark class="pad">`user@host:$`<mark class="command"> `tree2txt.sh` </mark>`datarun.root`</mark>

This will generate the file `datarun_T.txt`. Then we simply open Octave  
and run the script:

<mark class="pad">`user@host:~$`<mark class="command"> `octave`</mark></mark>
...
<mark class="pad">`user@host:~$`<mark class="command"> `tree2txt.sh` </mark> `datarun.root T_neutrons T_neutrons_extracted.txt`</mark>
