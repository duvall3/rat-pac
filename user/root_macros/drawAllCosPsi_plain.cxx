// drawAllCosPsi_plain -- macro to draw the cos[psi] results for all the specified *_reults.root files in a directory
...
TList* drawAllCosPsi_plain( const Bool_t kNormalized = kFALSE, const Bool_t kInteractive = kFALSE, const Bool_t kLaTeX = kTRUE ) {
...
TString resultsDir("/home/mark/COMPMAIN_RESULTS/ROOT_files/plain");
...
if (kNormalized) {
  ...
  const char *savename1 = "cos_psi_all_norm_3d.eps";
  const char *savename2 = "cos_psi_all_norm_2d.eps";
} else {
  /* const char *savename1 = "cos_psi_all_3d.png"; */
  /* const char *savename2 = "cos_psi_all_2d.png"; */
  const char *savename1 = "cos_psi_all_3d.eps";
  const char *savename2 = "cos_psi_all_2d.eps";
}
...
TString dLabel;
// mean markers
Double_t ylow, yup, yMarker, hMean, kMarkerOffset(0.003), kYLabelOffset(1.35);
...
//    summaryFile << NString.Data() << cpString.Data() << cpNString.Data();
  ...
  k++;
  cout << f->GetName() << " complete." << endl; //debug
...
if (kNormalized) {
  ...
  TText *ylabel3 = new TText(1.1, 0., "Relative Frequency");
  TText *ylabel2 = new TText(1.1, 0., "Relative Frequency");
} else {
...
h = (TH1D*)hList->At(0);
Double_t yLogMin = 1.e-2;
/* Double_t yLogMax = 10.**(TMath::Floor(TMath::Log10(allMax3))+1); */
/* Double_t yLogMax = 2 * allMax3; */
h->SetAxisRange(0., allMax3, "y");			// linear
//h->SetAxisRange(yLogMin, yLogMax, "y");		// log
//h->GetYaxis()->SetLimits(yLogMin, yLogMax);		// log
//h->GetYaxis()->SetRangeUser(yLogMin, yLogMax);	// log
//can_hcp_1->SetLogy(kTRUE);				// log
ylabel3->SetY(allMax3);
/* ylabel3->Draw("same"); */
/* h->GetYaxis()->SetTitle(ylabel3->GetTitle()); */
/* h->GetYaxis()->SetTitleOffset(kYLabelOffset); */
// 2D plot
can_hcp_2->cd();
...
/* ylabel2->Draw("same"); */
/* h->GetYaxis()->SetTitle(ylabel2->GetTitle()); */
/* h->GetYaxis()->SetTitleOffset(kYLabelOffset); */
