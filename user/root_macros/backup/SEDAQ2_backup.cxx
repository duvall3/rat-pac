// SEDAQ2_backup -- modified version of SimpleEnergyDAQ to operate on files that
...
void SEDAQ2_backup( const char* filename, const Bool_t kGraphics = kFALSE, const Bool_t kQuantizedPositions = kFALSE, Double_t prompt_low = 0, Double_t delayed_low = 0, Double_t deltaT_low = 1.e-6, Double_t deltaT_high = 100.e-6, const Bool_t kNuLat = kFALSE) {
...
if ( kGraphics == true ) { // draw graphics unless in batch mode (default false)
  ...
  for (( k=0; k < num_bursts; k++ )) {
    T_scint->GetEntry(k);
    if ( corrected_energy_q > 0 ) {
      h1->Fill(interevent_time);
      h2->Fill(interevent_time, corrected_energy_q); // time in s, energy in MeV
      h3->Fill(corrected_energy);
      h4->Fill(corrected_energy_q);
      h5->Fill(interevent_time, corrected_energy);
    }
  }
...
//Double_t xmin = 1.e-7; //s
  ...
  // draw histograms and add legend
