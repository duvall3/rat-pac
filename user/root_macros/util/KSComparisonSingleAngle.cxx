// KSComparisonSingleAngle -- small function to simplify use of TRefMatch class
...
TRefMatch* KSComparisonSingleAngle( Int_t testFileNumber, Double_t phiTrue = TMath::QuietNaN(), const char* branchName = "phiSeg", Long64_t nTestSampleEvents = 0, Long64_t nReferenceEvents = 0 ) {
...
// usage info
TString MsgLoc("KSComparisonSingleAngle");
TString UsageInfo;
UsageInfo.Form("\n\n\
\tKSComparisonSingleAngle( testFileNumber, phiTrue, nTestSampleEvents, nReferenceEvents )\n\
\t-- testFileNumber (required): index/ID from batch of single-angle testfiles\n
...
\t-- Examples:\n\
\t   -- Run the test for the data taken at phiTrue = 12 degrees: \"KSComparisonSingleAngle(0,12);\"\n\
\t   -- Same as above, but use only the first 100 events: \"KSComparisonSingleAngle(0,12, 100);\"\n\
\t   -- Same as above, but also use only the first 10 000 events from each reference dataset: \"KSComparisonSingleAngle(0,12, 100, 10000);\"\n");
...
testFileName.Form("TEST_SAMPLES/single_angle/12DEG_testsample%d.root", testFileNumber);
...
TRefMatch* KSComparisonSingleAngle( Int_t testFileNumber, Int_t phiTrue = TMath::QuietNaN(), const char* branchName = "phiSeg", Long64_t nTestSampleEvents = 0, Long64_t nReferenceEvents = 0 ) { return KSComparisonSingleAngle( (Double_t)phiTrue, branchName, nTestSampleEvents, nReferenceEvents ); }
