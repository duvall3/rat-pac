...
// init
TFile *f = TFile::Open("fitTest2_data.root");
TTree *T = T_scint;
...
TH1D *h = new TH1D("h", "IBD Interevent Time", 100, 0., tmax-tmin);
/* TH1D *h = new TH1D("h", "IBD Interevent Time", 100, 6., tmax-tmin); */
for ( ev=0; ev<N; ev++ ) {
  T->GetEntry(ev);
...
TFitResultPtr Rfc = h->Fit(fc, "SR");
TFitResult *rfg = Rfg.Get();
TFitResult *rfp = Rfp.Get();
TFitResult *rfc = Rfc.Get();
fg->Draw("same");
fp->Draw("same");
fc->Draw("same");
h->SetAxisRange(0.5, 2*h->GetMaximum(), "y");
gPad->SetLogy(1);
printf("\nGaussian Chi^2/NDF:\t%e\n\n", rfg->Chi2() / rfg->Ndf());
printf("\nPoisson Chi^2/NDF:\t%e\n\n", rfp->Chi2() / rfp->Ndf());
printf("\nCombined Chi^2/NDF:\t%e\n\n", rfc->Chi2() / rfc->Ndf());

// legend
TLegend *l = new TLegend(0.5, 0.2, 0.8, 0.4);
l->AddEntry(h, "Data");
l->AddEntry(fp, "Poisson (generation)", "L");
l->AddEntry(fg, "Gaussian (capture)", "L");
l->AddEntry(fc, "Combined", "L");
l->Draw();

// save plot
c1->Print("fitTest2.png");
